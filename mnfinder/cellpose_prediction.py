...
for i in range(len(validation_files)):
    ...
    save_path = DIRECTORY + 'cellpose_predictions_output/'
    np.save(save_path + imid + '._probabilities.npy', masks)
    
    # evaluation
    mn_gt = microdet.mnds.read_image(DIRECTORY, imid, 'phenotype_outlines.png', scale=SCALE_FACTOR)
    microdet.evaluation.segmentation_report(imid=imid, predictions=masks, gt=mn_gt, intersection_ratio=0.1, report_obj='Micronuclei')
